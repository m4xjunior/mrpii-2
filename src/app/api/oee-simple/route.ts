  ...
  try {
    const sql = `
      SELECT
        ...
        SUM(CASE WHEN hp.id_actividad = 2 THEN hp.unidades_repro ELSE 0 END) as total_rw,
      ...
      WHERE cm.Cod_maquina = '${machineId}'
        AND hp.fecha_fin >= '${startDate.toISOString()}'
        AND hp.fecha_fin <= '${endDate.toISOString()}'
  ...
  try {
    const sql = `
      SELECT
        ...
        MAX(ho.cod_of) as of_actual,
        MAX(cp.desc_producto) as producto_actual,
        MAX(ho.unidades_planning) as unidades_planificadas,
      ...
      LEFT JOIN cfg_producto cp ON ho.id_producto = cp.id_producto
      WHERE cm.Cod_maquina = '${machineId}'
        AND hp.fecha_fin >= '${startDate.toISOString()}'
        AND hp.fecha_fin <= '${endDate.toISOString()}'
