...
export async function GET(request: NextRequest) {
  try {
    ...
    // Consulta para obter dados por turno usando F_his_ct
    const sqlShifts = `
      SELECT
        fhc.desc_turno as turno,
        fhc.OEE_c as oee,
      ...
      WHERE cm.Cod_maquina = '${cod_maquina.replace(/'/g, "''")}'
        ...
        AND cm.Rt_Cod_of <> '--'
      GROUP BY fhc.desc_turno, fhc.OEE_c, fhc.Rend_c
      ORDER BY
        CASE fhc.desc_turno
          WHEN 'MAÑANA' THEN 1
          WHEN 'TARDE' THEN 2
    ...
    const shiftData = await executeQuery<any>(sqlShifts, undefined, 'mapex');

    // Processar dados dos turnos - já inclui OEE da F_his_ct
    ...
    for (const turno of turnos) {
      ...
      const shiftResult: ShiftData = {
        turno: turno,
        oee: Math.max(0, Math.min(100, turnoData.oee || 0)),
        rendimiento: Math.max(0, Math.min(100, turnoData.rendimiento || 0)),
