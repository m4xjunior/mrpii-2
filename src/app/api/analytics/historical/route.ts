...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
        ...
        WITH DailyData AS (
          SELECT
            ...
            ISNULL(AVG(CASE WHEN hp.id_actividad = 2 THEN
              CASE WHEN (hp.unidades_ok + hp.unidades_nok + hp.unidades_repro) > 0
                THEN DATEDIFF(SECOND, hp.fecha_ini, hp.fecha_fin) / (hp.unidades_ok + hp.unidades_nok + hp.unidades_repro)
                ELSE 0 END
            END), 0) as seg_por_pza_avg,
            ISNULL(AVG(CASE WHEN hp.id_actividad = 2 AND (hp.unidades_ok + hp.unidades_nok + hp.unidades_repro) > 0
              THEN 3600 / (DATEDIFF(SECOND, hp.fecha_ini, hp.fecha_fin) / (hp.unidades_ok + hp.unidades_nok + hp.unidades_repro))
      ...
      historicalData = await executeQuery<any>(sqlHistorical, undefined, 'mapex');
