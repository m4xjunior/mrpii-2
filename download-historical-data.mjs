...
// Función executeQuery simplificada
async function executeQuery(sql, params = null, database = 'mapex') {
  return new Promise((resolve, reject) => {
    ...
    const connection = new Connection({
      ...
      options: {
        ...
        database: database === 'mapex' ? (process.env.DB_NAME || 'mapexbp_Test') :
                 database === 'sage' ? 'SAGE' : 'WHALES',
      },
    });
    ...
    connection.on('connect', (err) => {
      ...
      if (params) {
        // Simplificado - por ahora no soportamos parámetros
        console.log('⚠️ Parámetros no soportados en esta versión simplificada');
      }
...
async function downloadHistoricalData() {
  ...
  try {
    ...
    // 2. DESCARGAR DATOS HISTÓRICOS SIMPLIFICADOS
    console.log('📈 Descargando datos históricos básicos...');
    let productionData = [];
    let downtimeData = [];

    try {
      // Consulta simplificada de producción (solo datos que existen)
      ...
      productionData = await executeQuery(productionQuery);
      console.log(`✅ Registros de producción: ${productionData.length}`);
    } catch (error) {
      console.warn('⚠️ Error descargando producción:', error.message);
    }

    try {
      // Consulta simplificada de paros
      const downtimeQuery = `
        SELECT TOP 100
          cm.Cod_maquina,
          hpp.fecha_ini,
      ...
      downtimeData = await executeQuery(downtimeQuery);
      console.log(`✅ Registros de paros: ${downtimeData.length}`);
    } catch (error) {
      console.warn('⚠️ Error descargando paros:', error.message);
    }
    ...
    Object.keys(dataByTime.by_minute).forEach(minuteKey => {
      ...
      const safeDateKey = date.replace(/\//g, '-'); // Convertir barras a guiones para nombres de archivo
      if (!days[safeDateKey]) days[safeDateKey] = {};
      days[safeDateKey][minuteKey] = dataByTime.by_minute[minuteKey];
    });
    ...
    for (const [date, dayData] of Object.entries(days)) {
      const dayFile = path.join(dataDir, `day_${date}.json`);
      await fs.writeFile(
        dayFile,
        JSON.stringify({
          date: date,
