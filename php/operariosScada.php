...
function mostrarInformacionOF($machineId, $cidm) {
    ...
    $sql_produccion_turno = "SELECT 
                COALESCE(cm.rt_desc_turno, 'Sin turno') as turno,
            ...
            GROUP BY COALESCE(cm.rt_desc_turno, 'Sin turno'), CONVERT(VARCHAR(10), hp.fecha_fin, 111)
    ...
    // Procesar datos para gráfico de producción por turno
    $labels_turno = [];
    $data_ok = [];
    $data_nok = [];
    $data_rw = [];
    ...
    // Si no hay datos, usar valores por defecto para evitar error
    if (empty($labels_turno)) {
        $labels_turno = ['No hay datos'];
        $data_ok = [0];
        $data_nok = [0];
        $data_rw = [0];
    }
...
function mostrarInformacionParos($machineId, $cidm) {
    ...
    function aplicarFiltrosParos(machineId) {
        ...
        var parosContainer = document.getElementById("paros-content-" + machineId);
        if (!parosContainer) {
