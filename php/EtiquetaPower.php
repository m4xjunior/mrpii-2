...
$cantidad = 1;//$_GET["cantidad"];

$num_eti = $_GET["serie"];
$serie2= $num_eti;
$serie= $num_eti;
$empresa = 1;//$_GET["empresa"];
$pieza = trim($_GET["pieza"]);
$articulo = $pieza;
$tipo = "PALET";//$_GET["tipo"];
$cajapalet = "PALET";//$_GET["tipo"];
...
$pdf->SetFont('Helvetica' , 'B' , 45* $tamanofuente); 
$pdf->Text( 6*$factorx+$despx,95 *$factory, $descripcion);
...
$pdf->Text( 6*$factorx+$despx,80 *$factory,  "MODEL: ".$articulo);
...
$pdf->Text( 150*$factorx+$despx, 10*$factory, $preffix.$numserie_max+$serie2);
...
*/
//$articulo="SANTOS".$articulo;
shell_exec("rm ../tmp/*");

 //file_put_contents('/var/www/MRPII/ofs/LOG_PRN_'.$posicion.$tipo.'.log', date("Ymd H:i:s")." ACTUALIZADO  barcode 3 \n", FILE_APPEND );
 $d1=new DNS1DBarcode();
 $d1->save_path="../tmp/";
 $d1->getBarcodePNGPath(trim($articulo), 'C128');




list($width, $height, $type, $attr) = getimagesize('../tmp/'.trim($articulo).'.png');
//$imagefactor=$width/$height;
$imagefactor=$width;
//$pdf->Text( 20*$factorx+$despx, 250 *$factory+$despy, 20,$imagefactor);
$pdf->Image('../tmp/'.trim($articulo).'.png',  8*$factorx+$despx,$alto - $height/4 - 22 *$factory+$despy,$imagefactor/1.5, 40);
...
$d1->save_path="../tmp/";
//echo $serie;
$d1->getBarcodePNGPath("S".$preffix.$numserie_max, 'C128');

list($width, $height, $type, $attr) = getimagesize('../tmp/'."S".$preffix.$numserie_max.'.png');
//if ($height==0) $height =1;
//$imagefactor=$width/$height;
$imagefactor=$width;
$pdf->Image('../tmp/'.'S'.$preffix.$numserie_max.'.png',  130*$factorx+$despx, 60 - $height/4 -0 *$factory,$imagefactor/1.5 , 30);   
