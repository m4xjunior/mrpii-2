...
$sqlins = "select ar.codigoarticulo , ar.descripcionarticulo , ac.codigocliente , ac.codigodelcliente , cl.razonsocial , cl.domicilio , cl.viapublica, cl.numero1 , cl.numero2 , cl.codigopostal , cl.provincia , cl.nacion , ar.kh_pe_peso , ar.kh_pe_pesopal , ar.ReferenciaEdi_ , cl.referenciadelproveedor , ar.marcaproducto , ar.mo_formaenvasado
...
and ar.codigoempresa = cl.codigoempresa and ac.codigocliente = cl.codigocliente and ac.codigocliente <> '0471' and ac.codigocliente <> '0324' and ac.codigodelcliente <> 'NR' ";
//echo $sqlins;
...
$serie = 0;//odbc_result($result,"odtnums");
$serie_ini = $serie;

$pneto = odbc_result($result,"kh_pe_peso");
$pbruto = round(odbc_result($result,"kh_pe_pesopal") , 5);
...
$embalaje = odbc_result($result,"mo_formaenvasado");
//$cliente = "FAURECIA";
...
$sqlS = "select  max(contador) from P_HistEtiquetasOF_Q1 WITH (NOLOCK) ";

 $sqlS = "SELECT sysContadorValor FROM lsysContadores WHERE sysNombreContador='Etiquetas' ";

$resultS=odbc_exec($cid,$sqlS) or die(exit("Error en odbc_exec")); 
$numserie_max =  odbc_result($resultS,1)+1; 
 $sqlS = "update  lsysContadores set sysContadorValor = $numserie_max  WHERE sysNombreContador='Etiquetas' ";
...
$sqlS = "insert into P_HistEtiquetasOF_Q1  ([Mo_Lote]
      ,[CodigoArticulo]
      ,[RazonSocial2]
      ,[DescripcionArticulo]
      ,[ReferenciaEdi_]
      ...
      ,[IdCliente_]
      ,[IS_EtiquetaDoble]
      ,[KH_LineFeedLoc2]) values ( '".$splitOF[0].$splitOF[1].$splitOF[2]."' , '$referencia' , 'KH Vives, S.L.' , '$descripcion' , '' , $cantidad , '' , '".date("Y/m/d H:i:s")."' , '$nivel' , '$referencia_cli' , '',  '$cliente' , '' , '$muelle' , '' ,  $pbruto , '' , $numserie_max , '$embalaje' , '$numcliente' , '' , '' , 0 , '' )";
//echo $sqlS."<br>";
$resultS=odbc_exec($cid,$sqlS) or die(exit("Error en odbc_exec"))+1; 


$sqlS = "select  max(numeroetiqueta) from KHITT_EtiquetasOF WITH (NOLOCK) where ordenfabricacion = '".$splitOF[0]."-".$splitOF[1]."-".$splitOF[2]."' and referencia = '$referencia'";

$resultS=odbc_exec($cid,$sqlS) or die(exit("Error en odbc_exec")); 



$serie =  odbc_result($resultS,1)+1; 




$sqlS = "insert into KHITT_EtiquetasOF values ( 1 , '".$splitOF[0]."-".$splitOF[1]."-".$splitOF[2]."|".$referencia."|C|".$serie."' , $splitOF[0] , '$splitOF[1]' ,$splitOF[2] ,$splitOF[3] ,$splitOF[4] , '".$splitOF[0]."-".$splitOF[1]."-".$splitOF[2]."' , '$referencia' , $numserie_max ,  '".date("Y/m/d H:i:s")."' , '' , 'C' , '' , '$operarios' , $serie , $cantidad , '$nivel'  )";
//echo $sqlS."<br>";
$resultS=odbc_exec($cid,$sqlS) or die(exit("Error en odbc_exec"))+1; 
...
$pdf->Text( 128*$factorx+$despx, $alto -9 *$factory+$despy, $of."|".substr($cajapalet,0,1)."|".($serie));
...
$pdf->Text( 27*$factorx+$despx, $alto -9 *$factory+$despy, $preffix.$numserie_max);
...
$pdf->SetFont('Helvetica' , 'B' , 25* $tamanofuente); 
$pdf->Text( 80*$factorx+$despx,$alto - 43*$factory+$despy,   "P5100");	
	$pdf->SetFont('Helvetica' , 'B' , 54* $tamanofuente); 
$pdf->Text( 108*$factorx+$despx,$alto - 43*$factory+$despy,   substr(  $referencia_cli , 5) );	
} else {
...
$of2 = "";
  QRcode::png($of."|".substr($cajapalet , 0 ,1)."|".trim($articulo)."|".($serie)."|".$cantidad."|".$numserie_max, '../tmp/BQR'.$of.'_'.$articulo.'_'.substr($cajapalet , 0 ,1).'_'.($serie).'.png'); 

    list($width, $height, $type, $attr) = getimagesize('../tmp/BQR'.$of.'_'.$articulo.'_'.substr($cajapalet , 0 ,1).'_'.($serie).'.png');
...
$imagefactor=$width/$height;
 ...
 //$pdf->Image('../tmp/'.$of2.'.png', 182*$factorx+$despx,   0*$factory+$despy+1,15*$factorx*$imagefactor);
  ...
  $pdf->Image('../tmp/BQR'.$of.'_'.$articulo.'_'.substr($cajapalet , 0 ,1).'_'.($serie).'.png', 178.5*$factorx+$despx, $alto - $height/2 -  0*$factory+$despy+1, 21*$factorx*$imagefactor);
...
//$pdf->ezStream(); 
	...
	$contenido_pdf = $pdf->output("../etiquetas/OF_".$orden."_".$pieza."_C_".$serie.".pdf" ,"F"); 
   $salida = shell_exec("lp -d CitizenTermo ../etiquetas/OF_".$orden."_".$pieza."_C_".$serie.".pdf  -o fit-to-page -o media=Custom.160x210mm \n");
?>
