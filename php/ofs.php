...
where  centrotrabajo = '".$posicion."'  and o.codigoempresa = 1 and c.codigoempresa = o.codigoempresa and o.formula = c.formula
...
echo "<th>CANT. PLA</th>";
echo "<th>CANT. REAL</th>";
echo "<th>ACTIVAR</th></tr></thead><tbody>";
...
//odbc_fetch_row($result);
	...
	$par = 1;
	echo "<tr ><td  >".odbc_result($result,'ejerciciofabricacion').odbc_result($result,'seriefabricacion').odbc_result($result,'numerofabricacion')."</td><td>".substr(odbc_result($result,'fechainicioprevista'),0,10)."</td><td>".substr(odbc_result($result,'horainicioprevista'),0,5)."</td><td>".odbc_result($result,'estadoof')."</td><td>".odbc_result($result,'CodigoArticulo')."</td><td>".odbc_result($result,'descripcionarticulo')."</td><td>".number_format(odbc_result($result,'UnidadesAFabricar'),0,",",".")."</td><td>".number_format(odbc_result($result,'UnidadesFabricadas'),0,",",".")."</td><td>"."<div id='activar_of' onclick='activar_of(\"".odbc_result($result,'ejerciciofabricacion').odbc_result($result,'seriefabricacion').odbc_result($result,'numerofabricacion')."\");' ><center><a class='ui-shadow ui-btn ui-corner-all'style='width: 80% ;  height: 20px; backgroundcolor: red; font-size: 100%; position: blocked; border:1px solid #CCC; background:#ADD; box-shadow: 0 0 5px -1px rgba(0,0,0,0.2); vertical-align:middle;  padding: 5px;  text-align: center;'>ACTIVAR</a></center></div>"."</td>";
