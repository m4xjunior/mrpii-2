          ...
          <div
            ...
            style={{
              background: isDark
                ...
                : 'linear-gradient(135deg, #673ab7 0%, #5a32a3 100%)',
              color: 'white',
              padding: getSpacing(0.8, 1, 1.5),
              flexShrink: 0,
              borderBottom: `1px solid ${isDark ? '#404040' : '#5a32a3'}`
            }}
          >
                ...
                {[
                  {
                    ...
                    chartData: generateTrendData(7, 65, 95) // Dados simulados para estado
                  },
                  {
                    ...
                    chartData: generateTrendData(7, machine.production.ok - 50, machine.production.ok + 50) // Dados simulados para produção OK
                  },
                  {
                    ...
                    chartData: generateTrendData(7, Math.max(0, machine.production.nok - 20), machine.production.nok + 30) // Dados simulados para produção NOK
                  },
                  {
                    ...
                    chartData: generateTrendData(7, Math.max(0, machine.efficiency - 15), Math.min(100, machine.efficiency + 10)) // Dados simulados para eficiência
                  }
                ].map((metric, index) => (
                ...
                {tabs.map((tab) => (
                  <button
                    ...
                    style={{
                      ...
                      boxShadow: activeTab === tab.id ? `0 4px 15px rgba(103, 58, 183, 0.3)` : 'none',
                      ...
                      border: isDark && activeTab !== tab.id ? `1px solid ${themeColors.border}` : 'none'
                    }}
                  >
                    <i
...
// Função para gerar dados de tendência simulados
function generateTrendData(days: number, minValue: number, maxValue: number): { date: string; value: number }[] {
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    // Gerar um valor aleatório dentro do intervalo, com uma tendência suave
